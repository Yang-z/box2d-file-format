...
auto db2Decoder::decode() -> void
{
    ...
    for (auto b = db2w.bodyList; b < db2w.bodyList + db2w.bodyCount; ++b)
    {
        ...
        */
        ...
        /*.userData*/ db2bs.userData.push_back(b2b);
        ...
        for (auto f = db2b.fixtureList; f <= db2b.fixtureList + db2b.fixtureCount - 1; ++f)
        {
            ...
            /*.userData*/ db2fs.userData.push_back(b2b->CreateFixture(&b2fdef));
            if (b2s)
    ...
    for (auto j = db2w.jointList; j < db2w.jointList + db2w.jointCount; ++j)
    {
        ...
        /*.userData*/ db2js.userData.push_back(b2w->CreateJoint(b2jdef));
        if (b2jdef)
            delete b2jdef;
    }
}
...
auto db2Decoder::encode() -> void
{
    ...
    for (int b = bodies.size() - 1; b >= 0; --b) // reverse order
    {
        ...
        for (int f = fixtures.size() - 1; f >= 0; --f) // reverse order
        {
            ...
            /*.userData*/ db2fs.userData.push_back(b2f);
            /*userData*/ b2f->GetUserData().pointer = (uintptr_t)db2fs.size() - 1;

            /*shape*/
            auto b2s = b2f->GetShape();
            /*add sub-chunk*/ auto &db2s = db2ss.emplace_back();
    ...
    for (auto b2j = this->b2w->GetJointList(); b2j; b2j = b2j->GetNext())
        joints.push_back(b2j);

    for (int j = joints.size() - 1; j >= 0; --j) // reverse order
    {
        auto &b2j = joints[j];

        /*add sub-chunk*/ auto &db2j = db2js.emplace_back();
        /*type*/ db2j.type3() = b2j->GetType();
        /*bodyA*/ db2j.emplace_back(int32_t(b2j->GetBodyA()->GetUserData().pointer));
        /*bodyB*/ db2j.emplace_back(int32_t(b2j->GetBodyB()->GetUserData().pointer));
        /*collideConnected*/ db2j.emplace_back(b2j->GetCollideConnected());

        /*.userData*/ db2js.userData.push_back(b2j);
        ...
        case b2JointType::e_weldJoint:
        {
            ...
            db2j.emplace_back(b2j_w->GetLocalAnchorB().x);
            db2j.emplace_back(b2j_w->GetLocalAnchorB().y);
            db2j.emplace_back(b2j_w->GetReferenceAngle());
            db2j.emplace_back(b2j_w->GetStiffness());
            db2j.emplace_back(b2j_w->GetDamping());
        }
        break;
